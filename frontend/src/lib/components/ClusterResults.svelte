<script lang="ts">
  ...
  async function confirmWholeCluster(cluster: any, tariffCode?: string, confidenceScore?: number) {
    ...
    console.log(`🔄 Starting confirmation for cluster ${clusterId} with tariff ${suggestion.tariff_code}`);
    ...
    console.log(`⚡ INSTANT VISUAL UPDATE! Cluster ${clusterId} added to visuallyConfirmedClusters`, Array.from(visuallyConfirmedClusters));
  ...
  function isClusterConfirmed(clusterId: string): boolean {
    ...
    const confirmed = visuallyConfirmedClusters.has(clusterId) || confirmedClusters.has(clusterId);
    if (confirmed) {
      console.log(`✅ isClusterConfirmed(${clusterId}) = TRUE (visual: ${visuallyConfirmedClusters.has(clusterId)}, confirmed: ${confirmedClusters.has(clusterId)})`);
    }
    return confirmed;
  }
  ...
  $: confirmationProgress = totalClusters > 0 ? (confirmedCount / totalClusters) * 100 : 0;
  
  // Create reactive map of confirmed statuses - this forces Svelte to re-render when confirmation state changes
  $: clusterConfirmedMap = clusters.reduce((map, c) => {
  ...
  <div class="p-6 border-b border-gray-200 bg-gray-50">
    ...
    {#if totalClusters > 0}
      <div class="mt-4 bg-white rounded-lg border border-gray-200 p-4">
        <div class="flex items-center justify-between mb-2">
          <span class="text-sm font-medium text-[#272425]">Bestätigungsfortschritt</span>
          <span class="text-sm font-semibold {confirmedCount === totalClusters ? 'text-green-700' : 'text-[#BB1E38]'}">
            {confirmedCount} / {totalClusters} Cluster bestätigt
          </span>
        </div>
        <div class="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
          <div 
            class="h-3 rounded-full transition-all duration-500 ease-out {confirmedCount === totalClusters ? 'bg-green-600' : 'bg-[#BB1E38]'}"
            style="width: {confirmationProgress}%"
          ></div>
        </div>
        {#if confirmedCount === totalClusters && totalClusters > 0}
          <p class="mt-2 text-sm text-green-700 font-medium flex items-center gap-2">
            <CheckCircle class="h-4 w-4" />
            Alle Cluster wurden bestätigt! Sie können nun zum Export fortfahren.
          </p>
        {:else if confirmedCount > 0}
          <p class="mt-2 text-sm text-[#6b6b6b]">
            Noch {totalClusters - confirmedCount} Cluster verbleibend
          </p>
        {/if}
      </div>
    {/if}
    ...
    <div class="mt-3 bg-blue-50 border border-blue-200 rounded-md p-3">
      <p class="text-xs text-blue-800">
        ...
        oder jeden Artikel einzeln mit Ihrer Wahl bestätigen. Bestätigte Cluster werden automatisch eingeklappt.
      </p>
    </div>
  </div>
  ...
  {:else}
    <div class="divide-y divide-gray-200">
      {#each clusters as cluster (cluster.cluster_id)}
        ...
        {@const confirmedTariff = (isConfirmed && clusterActiveSelections[cluster.cluster_id]) 
          ? clusterActiveSelections[cluster.cluster_id] 
          : cluster.tariff_suggestions?.[0]?.tariff_code}
        {@const confirmedConfidence = cluster.tariff_suggestions?.find(s => s.tariff_code === confirmedTariff)?.confidence_score}
        
        <div class="transition-all duration-500 {isConfirmed ? 'bg-gradient-to-r from-green-100 to-green-50 border-l-8 border-green-600 shadow-lg scale-[0.99]' : 'hover:bg-gray-50 border-l-4 border-transparent scale-100'}">
          <!-- Cluster Header -->
          <div class="px-6 py-5 flex items-center justify-between">
            ...
            >
              <div class="flex-shrink-0">
                {#if expandedClusters.has(cluster.cluster_id)}
                  <ChevronDown class="h-5 w-5 {isConfirmed ? 'text-green-700' : 'text-gray-500'}" />
                {:else}
                  <ChevronRight class="h-5 w-5 {isConfirmed ? 'text-green-700' : 'text-gray-500'}" />
                {/if}
              </div>
                ...
                {#if isConfirmed}
                  ...
                  <div class="bg-green-600 text-white rounded-lg p-3 shadow-md">
                    <div class="flex items-center gap-3 mb-2">
                      <CheckCircle class="h-7 w-7 flex-shrink-0 animate-pulse" />
                      <h3 class="text-xl font-bold">{cluster.cluster_name}</h3>
                      <span class="px-3 py-1 rounded-full text-xs font-bold bg-white text-green-700">
                        {cluster.item_count} ARTIKEL BESTÄTIGT
                      </span>
                    </div>
                    <div class="flex items-center gap-2 text-sm ml-10">
                      <span class="font-semibold">HS-Code:</span>
                      <span class="font-mono font-bold text-lg bg-white text-green-700 px-3 py-1 rounded">
                        {confirmedTariff}
                      </span>
                      {#if confirmedConfidence}
                        <span class="px-2 py-1 rounded text-xs font-bold bg-green-500 text-white">
            ...
            {#if isConfirmed}
              ...
              <div class="ml-4 inline-flex items-center gap-3 px-6 py-3 bg-green-600 text-white rounded-lg shadow-lg">
                <CheckCircle class="h-6 w-6 animate-pulse" />
                <div class="text-left">
                  <div class="font-bold text-sm">BESTÄTIGT</div>
                  <div class="text-xs opacity-90">{cluster.item_count} Artikel gespeichert</div>
                </div>
              </div>
          ...
          <!-- Expanded Content -->
          {#if expandedClusters.has(cluster.cluster_id)}
            <div class="px-6 pb-6 {isConfirmed ? 'bg-green-50/50' : 'bg-gray-50/50'}">
              {#if isConfirmed}
                <!-- Confirmed Cluster Info Banner -->
                <div class="mb-4 bg-green-100 border border-green-300 rounded-lg p-4">
                  <div class="flex items-center gap-3">
                    <CheckCircle class="h-6 w-6 text-green-700 flex-shrink-0" />
                    <div>
                      <p class="font-semibold text-green-900">Cluster vollständig bestätigt</p>
                      <p class="text-sm text-green-800 mt-1">
                        Alle {cluster.item_count} Artikel wurden mit HS-Code <span class="font-mono font-bold">{confirmedTariff}</span> bestätigt.
                        Die Bestätigungen wurden gespeichert und können beim Export verwendet werden.
                      </p>
                    </div>
                  </div>
                </div>
